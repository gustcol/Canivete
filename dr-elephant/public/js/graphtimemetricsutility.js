...
function plotter(graphData, jobDefList) {
    ...
    var yRange = d3.scale.linear().range([MARGINS.top + GRAPH_HEIGHT, MARGINS.top])
        .domain([0, d3.max(graphData, function (d) { return d.runtime + d.runtime/5; })])
        .nice(5);                                                    // Ensures a nice round value at the end of y axis

    // The graph function
    var lineFunc = d3.svg.line()
        .x(function (d) { return xRange(d.flowtime); })
        .y(function (d) { return yRange(d.runtime); })
        .interpolate('linear');
    ...
    var lineWastedFunc = d3.svg.line()
        .x(function (d) { return xRange(d.flowtime); })
        .y(function (d) { return yRange(d.waittime); })
        .interpolate('linear');
    ...
    graphContainer.append("svg:g")
        ...
        .attr("cy", function (d) { return yRange(d.runtime); } )
        .attr("r", 4);
    ...
    graphContainer.append("svg:g")
        ...
        .attr("cx", function (d) { return xRange(d.flowtime); } )
        .attr("cy", function (d) { return yRange(d.waittime); } )
        .attr("r", 4);
    ...
    graphContainer.append("text")
        ...
        .attr("dy", ".30em")
        .style("text-anchor", "end")
        .text(function(d) { return "Run Time" });
    ...
    graphContainer.append("text")
        ...
        .attr("dy", ".30em")
        .style("text-anchor", "end")
        .text(function(d) { return "Wait Time" });
    ...
    function mousemove(d) {
        ...
        tooltip.select("circle")
            ...
            .attr("transform", "translate(" + xRange(record.flowtime) + "," + yRange(record.runtime) +")");

        tooltip.select("circle.bluecircle")
            .style("display", "inline")
            .attr("transform", "translate(" + xRange(record.flowtime) + "," + yRange(record.waittime) +")");

        // Set position of tooltip.
        var x = xRange(record.flowtime) - (tooltipWidth) - 10;
        var y = yRange(record.runtime) - tooltip.select("body").style("height").replace("px", "")/2;
        ...
        if ((yRange(record.runtime) + tooltip.select("body").style("height").replace("px", "")/2) >= yRange(0)) {
            y = yRange(record.runtime) - tooltip.select("body").style("height").replace("px", "") - 10;
        }
